...
int checkConnection(int sock) {
	...
	if (send(sock, "ENC_SERVER", 11, 0) < 0) { error("SERVER: ERROR writing to socket"); }

	if (recv(sock, buffer, 11, 0) < 0) { error("SERVER: ERROR reading from socket"); }
	
	if (strcmp(buffer, "ENC_CLIENT") == 0) { //if the connection is with enc_client
		return 0;
	}
...
int main(int argc, char *argv[]){
	...
	while(1){
		...
		if (checkConnection(connectionSocket) == 1) {
			perror("SERVER: Port number not owned by ENC_CLIENT");
		}
			...
			if (childPid == -1) { //bad
				...
				int plainSize = recvsz(connectionSocket);
				...
				memset(plaintext, '\0', plainSize*sizeof(char));

				recvstr(connectionSocket, plaintext, plainSize);
				...
				sendstr(connectionSocket, ciphertext);
