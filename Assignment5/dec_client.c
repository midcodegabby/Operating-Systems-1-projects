...
int checkConnection(int sock) {
	...
	if (send(sock, "DEC_CLIENT", 11, 0) < 0) { error("CLIENT: ERROR reading from socket"); }

	if (strcmp(buffer, "DEC_SERVER") == 0) { //if the buffer and "1" are the same
		return 0;
	}
...
int main(int argc, char *argv[]) {
	...
	FILE *cipherFile = fopen(argv[1], "r"); //open for read only
	...
	int keySize, cipherSize, i;
	keySize = (int)kst.st_size; 
	cipherSize = (int)cst.st_size;
	...
	for (i = 0; i < cipherSize - 1; i++) { //for loop checks the plainfile 
		currChar[0] = getc(cipherFile); //retrieve the current char in the file
		
		//add the currChar to the key
		ciphertext[i] = currChar[0];
	...
	if (checkConnection(socketFD) == 1) { //if the connection is not with enc_server
		perror("CLIENT: Port number not owned by DEC_SERVER");
		exit(2);
	}

	//send the size of the plaintext message to the server
	sendsz(socketFD, cipherSize); 

	sendstr(socketFD, ciphertext);
	...
	recvstr(socketFD, plaintext, cipherSize);
